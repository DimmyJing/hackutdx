<script lang="ts">
	...
	function updateInstance(rejected: boolean) {
		reviews.update((old) =>
			old.map((el) => {
				if (el.diagnosisID === data.reviewID) {
					el.reviewStatus = rejected ? 'REJECTED' : 'REVIEWED';
				...
				<div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
					...
					<dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
						<textarea
							...
							bind:value={feedback}
						/>
						<div class="flex flex-row justify-end mt-2 gap-x-3">
							<button class="btn btn-primary" on:click={() => updateInstance(false)}>
								Accept
							</button>
							<button class="btn btn-error" on:click={() => updateInstance(true)}> Reject </button>
						</div>
					</dd>
				</div>
			</dl>
