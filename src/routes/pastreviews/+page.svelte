...
<div class="flex flex-col items-center w-full m-8 gap-y-4">
	{#each $reviews.filter((el) => el.reviewStatus !== 'UNREVIEWED') as review}
		<a
			href={'/pastreviews/' + review.diagnosisID}
			class="w-full max-w-4xl shadow-xl card {review.reviewStatus === 'REVIEWED'
				? 'bg-primary'
				: 'bg-error'} bg-neutral"
		>
			<div class="flex flex-row items-center justify-between w-full text-left card-body">
				<div class="flex flex-row items-center gap-x-2">
					...
					{#each review.tags as tag}
						<p
							class="rounded-md whitespace-nowrap mt-0.5 px-1.5 py-0.5 text-xs ring-1 ring-inset text-gray-600 bg-secondary font-medium"
						>
							{tag}
						</p>
					{/each}
